...
contract GasTest is BaseTest {
    ...
    function _makeEdges(uint160 seed) internal pure returns (StrandsAllowlistBatch.Edge[] memory e) {
    ...
    function test_Batched_IsExecutionNeutralVersusIndividualCalls() public {
        StrandsAllowlistBatch.Edge[] memory individual = _makeEdges(0x10000);
        StrandsAllowlistBatch.Edge[] memory batched = _makeEdges(0x20000);
        ...
        before = gasleft();
        token.setDestinations(batched, true);
        uint256 batchedGas = before - gasleft();

        vm.stopPrank();

        emit log_named_uint("execution gas, N individual calls", individualGas);
        emit log_named_uint("execution gas, 1 batched call   ", batchedGas);
        emit log_named_uint("tx base gas saved (21k * (N-1)) ", TX_BASE_GAS * (N - 1));
        emit log_named_uint("signatures saved                ", N - 1);
        ...
        // Every edge actually landed.
        for (uint256 i = 0; i < N; ++i) {
            assertTrue(token.allowedDestination(batched[i].holder, batched[i].destination));
        }
    }
    ...
    function test_ReRunningAppliedBatch_IsMuchCheaper() public {
        StrandsAllowlistBatch.Edge[] memory edges = _makeEdges(0x30000);

        vm.startPrank(admin);
        uint256 before = gasleft();
        token.setDestinations(edges, true);
        uint256 firstRun = before - gasleft();

        before = gasleft();
        token.setDestinations(edges, true); // every edge skipped
